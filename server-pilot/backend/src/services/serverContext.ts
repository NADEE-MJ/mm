...
export const executeArbitraryCommand = async (
  serverId: string,
  command: string,
  timeoutMs = 30000,
): Promise<CommandResult> => {
  ...
  if (server.type === "local") {
    return runCommand(["/bin/zsh", "-lc", command], { timeoutMs, allowNonZero: true });
  }

  return sshPool.exec(server.id, command, timeoutMs);
};
