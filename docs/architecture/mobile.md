...
├── Config/
│   ├── App.xcconfig                   # Build settings (Debug + Release)
│   └── Env.generated.xcconfig         # Generated from .env (gitignored)
├── scripts/
│   └── generate-env-xcconfig.sh       # Reads .env → generates xcconfig
├── Sources/
│   ├── MovieManagerApp.swift           # App entry point, scene lifecycle
│   ├── Models/
│   │   ├── TabItem.swift              # Tab navigation model
│   │   └── ScrollState.swift          # Scroll position state
│   ├── Services/
│   │   ├── AuthManager.swift          # JWT auth, Keychain storage, offline handling
│   │   ├── NetworkService.swift       # REST API client, movie/person structs
│   │   ├── DatabaseManager.swift      # GRDB SQLite, migrations, caching
│   │   ├── WebSocketManager.swift     # Real-time sync (WebSocket)
│   │   ├── MovieRepository.swift      # Local-first data layer (repository pattern)
│   │   └── SyncManager.swift          # Pending operation queue, enrichment
│   ├── Views/
│   │   ├── RootTabHostView.swift      # Tab bar, search button, FAB
│   │   ├── Tabs/
│   │   │   ├── HomePageView.swift     # Movie list by status
│   │   │   ├── ListsPageView.swift    # Custom lists
│   │   │   ├── PeoplePageView.swift   # Recommenders + quick recommenders
│   │   │   ├── AccountPageView.swift  # Settings, backup, developer tools
│   │   │   └── ExplorePageView.swift  # TMDB search + add movie
│   │   ├── AddMoviePageView.swift     # Full add-movie sheet
│   │   ├── GlobalSearchPageView.swift # Cross-tab search
│   │   └── Components/
│   │       └── CachedAsyncImage.swift # URLCache-backed image loader
│   ├── Theme/
│   │   └── AppTheme.swift             # Colors, fonts, glass effects
│   └── Info.plist                     # App config (API URL via build settings)
└── MovieManager.xcodeproj              # Generated by XcodeGen (gitignored)
```
...
- If the call returns a **network error** (URLError / no connectivity) → keep session alive, allow offline use
- If the call returns **401/403** → call `logout()`, clear Keychain, force re-login
...
- `Movie` — full movie record including TMDB data, recommendations, watch history
- `Person` — recommender with `quick_key`, `color`, `emoji`
...
- `v1` — Initial schema
- `v2_full_movie_cache` — Adds `imdb_id` + `json_data` columns to movies; clears old cache
- `v2_pending_operations` — Creates `pending_operations` table
- `v2_pending_movies` — Creates `pending_movies` table
- `v3_people_quick_key_color_emoji` — Adds `quick_key`, `color`, `emoji` to people
...
1. Read cached records from GRDB → return immediately to UI
...
2. If it fails with a network error → store in `pending_operations` queue in GRDB
...
- `processPendingOperations()` — replays queued actions against the API
- `enrichPendingMovies()` — for movies added offline with only a title, searches TMDB and adds the matched movie with a proper TMDB ID
...
- **Cache policy**: `returnCacheDataElseLoad` — serve from cache whenever available
...
XcodeGen reads `project.yml` to generate `MovieManager.xcodeproj`. The generated project file is **gitignored**. Developers must run `xcodegen generate` after cloning.

```bash
brew install xcodegen
cd mobile
./scripts/generate-env-xcconfig.sh  # generates Config/Env.generated.xcconfig
xcodegen generate                   # generates MovieManager.xcodeproj
...
The search bar is hidden by default and triggered by a magnifying glass icon in the tab bar — between the tab pill and the FAB (+) button.

```
Default:   │  🏠  👥  📋  ⚙️    🔍  ➕  │
Expanded:  │ 🔍 Search movies...    ✕   │  ← above tab bar
           │  🏠  👥  📋  ⚙️    ✕  ➕  │
```
...
- Uses `os.Logger` with subsystem `com.moviemanager.app`
...
| Test | Steps | Expected |
|---|---|---|
| Offline launch | Login online → airplane mode → force quit → relaunch | Movie list loads from cache; no login screen |
| Offline data access | Browse online (caches data) → airplane mode → pull to refresh | Cached data shown; no crash |
| Offline write → sync | Go offline → change movie status → restore internet | Queued op syncs on next foreground |
...
| Image caching | View posters online → airplane mode → revisit | Posters load instantly from disk cache |
